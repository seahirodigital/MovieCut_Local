...
echo
echo "[*] JINRI mac 一括自動不要部削除 1 本化を開始します..."
echo
...
if [ "$BATCH_EXIT_CODE" -eq 0 ]; then
  echo "[OK] JINRI mac 一括自動不要部削除 1 本化は正常終了しました。"
else
  echo "[ERROR] JINRI mac 一括自動不要部削除 1 本化が失敗しました。終了コード: ${BATCH_EXIT_CODE}"
fi
